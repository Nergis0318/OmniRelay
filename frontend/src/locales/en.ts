export default {
  common: {
    cancel: "Cancel",
    create: "Create",
    update: "Update",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    refresh: "Refresh",
    apply: "Apply",
    done: "Done",
    search: "Search",
    loading: "Loading...",
    noData: "No data",
    confirm: "Confirm",
    close: "Close",
    back: "Back",
    signOut: "Sign out",
  },
  auth: {
    loginTitle: "Welcome back",
    loginSubtitle: "Sign in to the gateway console",
    registerTitle: "Create account",
    registerSubtitle: "Set up your gateway console access",
    username: "Username",
    email: "Email",
    password: "Password",
    confirmPassword: "Confirm Password",
    signIn: "Sign In",
    createAccount: "Create Account",
    noAccount: "No account?",
    createOne: "Create one",
    haveAccount: "Already have an account?",
    signInLink: "Sign in",
    invalidCredentials: "Invalid credentials",
    registrationFailed: "Registration failed",
    passwordsDontMatch: "Passwords don't match",
    passwordRequirements: "At least 6 characters",
  },
  nav: {
    dashboard: "Dashboard",
    providers: "Providers",
    models: "Models",
    apiKeys: "API Keys",
    usage: "Usage",
    logs: "Logs",
    performance: "Performance",
    passthrough: "Passthrough",
    gateway: "Gateway",
    users: "Users",
  },
  dashboard: {
    title: "Dashboard",
    subtitle: "Inference gateway overview",
    todayCost: "Today Cost",
    todayRequests: "Today Requests",
    todayTokens: "Today Tokens",
    total: "Total",
    performance: "Performance",
    avgLatency: "Avg Latency",
    totalRequests: "Total Requests",
    totalTokens: "Total Tokens",
    totalCost: "Total Cost",
    activeProviders: "Active Providers",
    registeredModels: "Registered Models",
    activeApiKeys: "Active API Keys",
    usage30Days: "Usage — 30 days",
    tokens: "Tokens",
    cost: "Cost",
    costAxis: "Cost ($)",
    noUsageData: "No usage data yet",
    system: "System",
    operational: "Operational",
  },
  providers: {
    title: "Providers",
    subtitle: "Manage upstream LLM provider connections",
    addProvider: "Add Provider",
    editProvider: "Edit Provider",
    key: "Key",
    name: "Name",
    type: "Type",
    status: "Status",
    actions: "Actions",
    active: "Active",
    inactive: "Inactive",
    providerKey: "Provider Key",
    displayName: "Display Name",
    apiBaseUrl: "API Base URL",
    apiKey: "API Key",
    providerType: "Provider Type",
    leaveEmpty: "(leave empty to keep)",
    autoSync: "Auto-sync models after save",
    autoSyncHint: "Fetch available models from provider API",
    syncedModels: "Synced {count} models",
    saveFailed: "Failed to save",
    syncFailed: "Sync failed",
    test: "Test Connection",
    testSuccess: "OK — {latency}ms",
    testFailed: "Test failed",
    noProviders: "No providers yet. Add one to get started.",
    deleteConfirm: "Delete this provider and all its models?",
    sourceModels: "Source Models",
    selectModelsHint: "Select models from existing providers to include",
    noSourceModels: "No models available from other providers",
    showInModelList: "Show in public model list",
    showInModelListHint: "When enabled, these models appear in /v1/models",
    additionalFormats: "Additional API formats",
    additionalFormatsHint: "Route a request format to its own upstream address (optional)",
    addFormat: "Add format",
  },
  models: {
    title: "Models",
    subtitle: "Registered inference models and pricing",
    addModel: "Add Model",
    editModel: "Edit Model",
    model: "Model",
    provider: "Provider",
    source: "Source",
    pricing: "Pricing ($/1M tok)",
    context: "Context",
    manual: "Manual",
    auto: "Auto",
    pricingIn: "in",
    pricingOut: "out",
    pricingCache: "cache",
    modelId: "Model ID",
    displayName: "Display Name",
    inputPrice: "Input $/1M",
    outputPrice: "Output $/1M",
    cacheWrite5m: "Cache Write 5m",
    cacheWrite1h: "Cache Write 1h",
    cacheRead: "Cache Read",
    contextWindow: "Context Window",
    selectProvider: "Please select a provider",
    saveFailed: "Failed to save",
    noModels: "No models registered. Add a provider first, then sync.",
    deleteConfirm: "Delete this model?",
  },
  apiKeys: {
    title: "API Keys",
    subtitle: "Manage gateway access credentials",
    issueKey: "Issue Key",
    name: "Name",
    keyPrefix: "Key Prefix",
    status: "Status",
    rateLimit: "Rate Limit",
    lastUsed: "Last Used",
    created: "Created",
    active: "Active",
    revoked: "Revoked",
    never: "Never",
    issueNew: "Issue New API Key",
    keyName: "Key Name",
    rateLimitRpm: "Rate Limit (RPM)",
    rateLimitHint: "Requests per minute — 0 means unlimited",
    createFailed: "Failed to create key",
    keyCreated: "Key Created",
    saveKeyNote: "Save this key — it won't be shown again.",
    noKeys: "No API keys issued yet.",
    revokeConfirm: "Revoke this API key?",
  },
  usage: {
    title: "Usage",
    subtitle: "Aggregated totals and 30-day chart",
    cacheWrite5m: "Cache Write 5m",
    cacheWrite1h: "Cache Write 1h",
    cacheRead: "Cache Read",
  },
  users: {
    title: "Users",
    subtitle: "Registered gateway users",
    username: "Username",
    email: "Email",
    role: "Role",
    admin: "Admin",
    member: "Member",
    created: "Created",
    noUsers: "No users registered yet.",
    toggleRole: "Toggle role",
    resetPassword: "Reset password",
    providers: "Provider access",
    delete: "Delete user",
    deleteTitle: "Delete User",
    deleteConfirm: "Are you sure you want to delete \"{name}\"? This cannot be undone.",
    resetTitle: "Password Reset Code",
    resetNote: "Share this one-time code with the user. It expires in 15 minutes.",
    providersTitle: "Provider Access",
    providersHint: "Select which providers this user can access. Leave empty to allow all.",
    noProviders: "No providers configured.",
  },
  performance: {
    title: "Performance",
    subtitle: "Throughput · Latency · TTFT",
    rpm: "RPM (req/min)",
    tpm: "TPM (tok/min)",
    avgLatency: "Avg Latency (DAVG)",
    ttft: "Avg TTFT",
    ttftHint: "Streaming only",
    p50: "P50",
    p95: "P95",
    p99: "P99",
    errorRate: "Error Rate",
    cacheHitRate: "Cache Hit Rate",
    timeseries: "Timeline",
    throughput: "Throughput",
    latency: "Latency",
    byProvider: "By Provider",
    byModel: "By Model",
    topModels: "Top Models by Cost",
    noData: "No data for this filter",
    model: "Model",
    provider: "Provider",
    requests: "Requests",
    tokens: "Tokens",
    cost: "Cost",
    granularity: "Granularity",
    from: "From",
    to: "To",
    auto: "Auto",
    streamingOnly: "Streaming only",
    noStreaming: "No streaming data",
    allProviders: "All providers",
  },
  passthrough: {
    title: "URL Passthrough",
    subtitle: "Relay latency without translation · records the usage upstream reports on its own responses",
    live: "LIVE",
    host: "Upstream host",
    hostPlaceholder: "api.openai.com",
    from: "From",
    to: "To",
    granularity: "Granularity",
    auto: "Auto",
    requests: "Requests",
    perSecond: "req/s",
    perMinute: "req/min",
    avgTotal: "Total time",
    avg: "Avg",
    p50: "P50",
    p95: "P95",
    p99: "P99",
    ttfb: "TTFB",
    ttfbHint: "to headers",
    prefill: "Upstream wait",
    ttft: "TTFT",
    streamOnly: "Streaming only",
    streaming: "streaming",
    noStreaming: "No streaming data",
    setup: "Connection cost",
    setupHint: "DNS+TCP+TLS",
    errorRate: "Error rate",
    avgResponse: "Avg response",
    responseSize: "Response",
    tokens: "Tokens",
    tokensHint: "as reported by upstream",
    inTok: "In tok",
    outTok: "Out tok",
    cacheTok: "Cache W/R",
    cacheRead: "Cache read",
    tokenFlow: "Token flow",
    ladder: "Latency breakdown",
    ladderNote:
      "Every bar shares one 0 – total time scale. Phase bars average only the requests where that phase actually happened, so a reused keep-alive connection records no DNS, TCP or TLS. TTFB and TTFT are cumulative from the start of the request.",
    groupPhases: "Average per phase",
    groupTotals: "Total time distribution",
    timeseries: "Over time",
    byHost: "By host",
    recent: "Recent measurements",
    recentNote: "Hover an error status code to read the cause",
    notEnoughPoints: "Only one measurement point so far - the trend needs a little more traffic",
    filterByHost: "Filter by this host",
    errors: " errors",
    time: "Time",
    path: "Path",
    status: "Status",
    total: "Total",
    of: " of ",
    prevPage: "Previous page",
    nextPage: "Next page",
    noData: "No measurements for these filters",
    clearFilters: "Clear filters",
    noRecords: "No measurements recorded",
    emptyTitle: "No relayed requests yet",
    emptyBody:
      "Send a request with the upstream URL embedded in the path and it lands here. This path does not use provider keys, so the caller must supply its own upstream authorization header.",
    copyCommand: "Copy example request",
    copied: "Copied",
  },
  logs: {
    title: "Request Logs",
    subtitle: "Per-request detail with token breakdown",
    model: "Model",
    provider: "Provider",
    from: "From",
    to: "To",
    startedAt: "Started At",
    completedAt: "Completed At",
    duration: "Duration",
    inputTokens: "Input",
    outputTokens: "Output",
    cacheWrite5m: "Cache W5m",
    cacheWrite1h: "Cache W1h",
    cacheRead: "Cache Read",
    totalTokens: "Total",
    totalCost: "Cost",
    totalRequests: "Total Requests",
    status: "Status",
    error: "Error",
    ok: "OK",
    noRecords: "No request logs found.",
    of: "of",
    records: "records",
    modelFilter: "Filter by model...",
    providerFilter: "Filter by provider...",
  },
};
