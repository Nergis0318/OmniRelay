...
func TestMigrationCreatesProviderKeysWhenAlreadyPastV14(t *testing.T) {
	db := openTestDB(t)
	...
	var enc string
	var active int
	if err := db.QueryRow(`SELECT api_key_encrypted, is_active FROM provider_api_keys`).Scan(&enc, &active); err != nil {
		t.Fatalf("copied row: %v", err)
	}
	if enc != "enc-abc" || active != 1 {
		t.Fatalf("got enc=%q active=%d", enc, active)
	}
}
